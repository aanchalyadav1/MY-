...
(async function(){
  if(!window.firebase){
    console.warn('Firebase SDK not loaded; admin.js can still render UI but will not connect to Firebase.');
  }
  ...
  function startEdit(type, index){
    editing = { type, index };
    if(type==='cert'){ const it = certs[index] || {}; certTitle.value = it.title||''; certIssuer.value = it.issuer||''; certLink.value = it.link||''; certPreview.innerHTML = it.image ? `<img src="${it.image}" class="preview-img">` : 'Preview'; addCert.style.display='none'; updateCertBtn.style.display='inline-block'; toggle(certFormWrap, true); window.scrollTo({top:0,behavior:'smooth'}); }
    if(type==='intern'){ const it = interns[index]||{}; internCompany.value = it.company||''; internRole.value = it.role||''; internDuration.value = it.duration||''; addIntern.style.display='none'; updateInternBtn.style.display='inline-block'; toggle(internFormWrap, true); window.scrollTo({top:0,behavior:'smooth'}); }
    if(type==='proj'){ const it = projects[index]||{}; projName.value = it.name||''; projDesc.value = it.desc||''; projUrl.value = it.url||''; projPreview.innerHTML = it.image ? `<img src="${it.image}" class="preview-img">` : 'Preview'; addProj.style.display='none'; updateProjBtn.style.display='inline-block'; toggle(projFormWrap, true); window.scrollTo({top:0,behavior:'smooth'}); }
  }
  ...
  function confirmDelete(type, index){
    ...
    confirmText.textContent = 'Are you sure you want to delete this item? This cannot be undone.';
    confirmModal.classList.add('show');
  }

  confirmNo.addEventListener('click', ()=> { pendingDelete = null; confirmModal.classList.remove('show'); });
  confirmYes.addEventListener('click', async ()=>{
    ...
    try{
      ...
      toast('Deleted');
    }catch(e){ toast('Delete failed', {type:'error'}); }
    pendingDelete = null; confirmModal.classList.remove('show');
  });

  // CRUD operations
  async function addCertificate(){
    const title = (certTitle.value||'').trim(); if(!title) return toast('Title required', {type:'error'});
  ...
  async function addIntern(){ const company = (internCompany.value||'').trim(); if(!company) return toast('Company required', {type:'error'}); addIntern.disabled=true;
    try{ const ref = db.collection('internships').doc('list'); await db.runTransaction(async tx=>{ const d = await tx.get(ref); const items = d.exists && Array.isArray(d.data().items) ? d.data().items.slice() : []; items.push({ company, role: internRole.value.trim(), duration: internDuration.value.trim(), addedAt: Date.now() }); tx.set(ref, { items }, { merge:true }); }); toast('Internship added'); cancelEditAll(); await renderInternships(); }catch(e){ console.error(e); toast('Add failed', {type:'error'}); } addIntern.disabled=false; }

  async function addProject(){ const name = (projName.value||'').trim(); if(!name) return toast('Project name required', {type:'error'}); addProj.disabled=true;
    try{ const file = projImage.files[0]; const url = file ? await uploadFile(file,'projects') : null; const ref = db.collection('projects').doc('list'); await db.runTransaction(async tx=>{ const d = await tx.get(ref); const items = d.exists && Array.isArray(d.data().items) ? d.data().items.slice() : []; items.push({ name, desc: projDesc.value.trim(), url: projUrl.value.trim() || null, image: url || null, addedAt: Date.now() }); tx.set(ref, { items }, { merge:true }); }); toast('Project added'); cancelEditAll(); await renderProjects(); }catch(e){ console.error(e); toast('Add failed', {type:'error'}); } addProj.disabled=false; }

  async function updateCertificate(){ const idx = editing.index; if(idx<0) return cancelEditAll(); updateCertBtn.disabled=true;
    try{ const ref = db.collection('certificates').doc('list'); await db.runTransaction(async tx=>{ const d = await tx.get(ref); const items = d.exists && Array.isArray(d.data().items) ? d.data().items.slice() : []; const file = certImage.files[0]; const url = file ? await uploadFile(file,'certificates') : items[idx].image || null; items[idx] = { ...items[idx], title: certTitle.value.trim(), issuer: certIssuer.value.trim(), link: certLink.value.trim() || null, image: url, updatedAt: Date.now() }; tx.set(ref, { items }); }); toast('Certificate updated'); cancelEditAll(); await renderCertificates(); }catch(e){ console.error(e); toast('Update failed', {type:'error'}); } updateCertBtn.disabled=false; }

  async function updateIntern(){ const idx = editing.index; if(idx<0) return cancelEditAll(); updateInternBtn.disabled=true; try{ const ref = db.collection('internships').doc('list'); await db.runTransaction(async tx=>{ const d = await tx.get(ref); const items = d.exists && Array.isArray(d.data().items) ? d.data().items.slice() : []; items[idx] = { ...items[idx], company: internCompany.value.trim(), role: internRole.value.trim(), duration: internDuration.value.trim(), updatedAt: Date.now() }; tx.set(ref, { items }); }); toast('Internship updated'); cancelEditAll(); await renderInternships(); }catch(e){ console.error(e); toast('Update failed', {type:'error'}); } updateInternBtn.disabled=false; }

  async function updateProject(){ const idx = editing.index; if(idx<0) return cancelEditAll(); updateProjBtn.disabled=true; try{ const ref = db.collection('projects').doc('list'); await db.runTransaction(async tx=>{ const d = await tx.get(ref); const items = d.exists && Array.isArray(d.data().items) ? d.data().items.slice() : []; const file = projImage.files[0]; const url = file ? await uploadFile(file,'projects') : items[idx].image || null; items[idx] = { ...items[idx], name: projName.value.trim(), desc: projDesc.value.trim(), url: projUrl.value.trim() || null, image: url, updatedAt: Date.now() }; tx.set(ref, { items }); }); toast('Project updated'); cancelEditAll(); await renderProjects(); }catch(e){ console.error(e); toast('Update failed', {type:'error'}); } updateProjBtn.disabled=false; }

  async function deleteCertificate(i){ const ref = db.collection('certificates').doc('list'); await db.runTransaction(async tx=>{ const d = await tx.get(ref); const items = d.exists && Array.isArray(d.data().items) ? d.data().items.slice() : []; items.splice(i,1); tx.set(ref, { items }); }); await renderCertificates(); }
  async function deleteIntern(i){ const ref = db.collection('internships').doc('list'); await db.runTransaction(async tx=>{ const d = await tx.get(ref); const items = d.exists && Array.isArray(d.data().items) ? d.data().items.slice() : []; items.splice(i,1); tx.set(ref, { items }); }); await renderInternships(); }
  async function deleteProject(i){ const ref = db.collection('projects').doc('list'); await db.runTransaction(async tx=>{ const d = await tx.get(ref); const items = d.exists && Array.isArray(d.data().items) ? d.data().items.slice() : []; items.splice(i,1); tx.set(ref, { items }); }); await renderProjects(); }

  // admin helpers
  async function exportData(){ const payload = { certificates: await fetchDocArray('certificates'), internships: await fetchDocArray('internships'), projects: await fetchDocArray('projects'), exportedAt: Date.now() }; const blob = new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'export.json'; a.click(); URL.revokeObjectURL(url); toast('Export started'); }
  async function recalcTimestamps(){ toast('Recalc started'); await Promise.all(['certificates','internships','projects'].map(async col=>{ const ref = db.collection(col).doc('list'); const d = await ref.get(); if(!d.exists) return; const items = d.data().items.map(it=> ({ ...it, syncedAt: Date.now() })); await ref.set({ items }, { merge:true }); })); toast('Recalc finished'); }
  ...
  if(auth && db){
    auth.onAuthStateChanged(async user => {
      ...
      // check admin list doc. If using custom claims, check token claims instead.
      try{
    ...
    // no firebase — still render skeleton placeholders
